...
sed -i "${LINE}c 127.0.0.1\t${HOST} localhost" /etc/hosts
LINE=`grep -n 127.0.1.1 /etc/hosts | head -1 | cut -d : -f 1`
sed -i "${LINE}c 127.0.1.1\t${HOST}" /etc/hosts
...
debconf-set-selections <<< "dovecot-core dovecot-core/create-ssl-cert boolean false"
